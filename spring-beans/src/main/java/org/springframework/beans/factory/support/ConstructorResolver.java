...
class ConstructorResolver {
	 ...
	 *
	 * 确定工厂对象，然后获取构造函数和构造参数，最后调用 InstantiationStrategy 对象的
	...
	private static class ArgumentsHolder {
		...
		public void storeCache(RootBeanDefinition mbd, Executable constructorOrFactoryMethod) {
			synchronized (mbd.constructorArgumentLock) {
				mbd.resolvedConstructorOrFactoryMethod = constructorOrFactoryMethod;//缓存已经解析的构造函数或者工厂方法。
				mbd.constructorArgumentsResolved = true; // 标记字段，标记构造函数、参数已经解析了。默认为 `false` 。
				if (this.resolveNecessary) {
					mbd.preparedConstructorArguments = this.preparedArguments; // 缓存已经解析的构造函数参数，包可见字段。
				}
				else {
